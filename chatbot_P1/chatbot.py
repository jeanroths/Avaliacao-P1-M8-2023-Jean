...
def chatbot_prompt():
    while True:
        ...
        if action == "Atualizar_pagamento" or action == "Acompanhar_pedido":
...
def chat_responses(intent, command):
    if intent:
        if intent in ['Atualizar_pagamento', 'Acompanhar_pedido']:
            for key, value in intent_dict.items():
                pattern = re.compile(key)
                groups = pattern.findall(command)
                if groups:
                    response = action_dict[value](groups[0])
                    if response:
                        print(response, end="")
        ...
        print('Desculpa ai, não entendi o que você disse, faz o L')
